#logitech c270, c310
#wifi nya beli realtek

import RPi.GPIO as GPIO
import threading
import time
import Queue

MERAH 	= 18
KUNING 	= 17
HIJAU 	= 22
BUTTON1 = 23
BUTTON0 = 27

GPIO.setmode(GPIO.BCM)			#BCM > (standart penulisan GPIO6)
					#BOARD > (standart penulisan PIN31)
GPIO.setup(MERAH,GPIO.OUT)
GPIO.setup(KUNING,GPIO.OUT)
GPIO.setup(HIJAU,GPIO.OUT)

GPIO.setup(BUTTON0,GPIO.IN)
GPIO.setup(BUTTON1,GPIO.IN)

GPIO.output(HIJAU,GPIO.HIGH)

lock = threading.Lock()				#satu object, untuk satu periperal
workQueue = Queue.Queue(10)
buttonQueue = Queue.Queue(10)


def loopingMerah():
	while True:
		try:
			m = raw_input("Masukan Teks x 100ms: ")	#mengambil masukan variabel pada terminal menjadi teks
			mm = float(m)/10.0
			workQueue.put(mm)		#menaruh data m pada variable queue
			time.sleep(0.5)			#kasih sedikit jeda buat program lain selesai
		except:
			print "sistem cant not converting input variable"
def loopingKuning():
	rate = 0
	while True:
		if not workQueue.empty():
			rate=workQueue.get()	#ambil data variable queue
			print rate
		if rate == 0.0:
			GPIO.output(MERAH,GPIO.HIGH)
		else:
			GPIO.output(MERAH,GPIO.HIGH)
			time.sleep(rate)
			GPIO.output(MERAH,GPIO.LOW)
			time.sleep(rate)
			
def loopingButton0():
	rate = 0.5
	while True:
		if not buttonQueue.empty():
			rate = buttonQueue.get();
		if GPIO.input(BUTTON0) == GPIO.LOW:
			rate-=0.1
			if rate <= 0.1:
				rate = 0.1
			workQueue.put(rate)
			while GPIO.input(BUTTON0) == GPIO.LOW:
				time.sleep(0.01)
		elif GPIO.input(BUTTON1) == GPIO.LOW:
			rate+=0.1
			if rate >= 0.9:
				rate = 0.9
			workQueue.put(rate)
			while GPIO.input(BUTTON1) == GPIO.LOW:
				time.sleep(0.01)
		time.sleep(0.01)	#membuat utilitas CPU lebih turun

thread1 = threading.Thread(target = loopingMerah)
thread1.daemon = True				#kalau daemon=True, maka thread (menjadi service) bisa di close jika program utama di close
						#kalau daemon=False, maka thread (menjadi service) tidak bisa di close saat program utama di close
thread1.start()

thread2 = threading.Thread(target = loopingKuning)
thread2.daemon = True
thread2.start()

thread3 = threading.Thread(target = loopingButton0)
thread3.daemon = True
thread3.start()

try:
	while True:
		time.sleep(0.01)
		#pass				#kalau di C itu NOP = No Operation

except KeyboardInterrupt:
	GPIO.cleanup()				#mereset GPIO menjadi floating point
	print "Selesai"