	...
	try:
		...
		if mdata['roomA'][0]['lampu']=='merah':
			if mdata['roomA'][0]['status']=='ON':
				...
				f = open("statusNiam.txt","a")						#kalau append(ditulis berkelanjutan) = f = open("status4.txt","a"), kalau di tindas f = open("status4.txt","w")	
				...
				ftp.login("anonymous","niamtamami") #kalau anonymous, maka password tidak di perhitungkan
				filenameNiam = "statusNiam.txt"
				ftp.storlines("STOR "+filenameNiam, open(filenameNiam))	# ftp.storlines("STOR /direktori/"+filenameNiam, open(filenameNiam)) >> masuk direktori
				ftp.quit()
				db = MySQLdb.connect("localhost","root","boss","niamtamami") # buat koneksi, db = MySQLdb.connect("alamathost","username","password","namatabel") 
				cursor = db.cursor()
				sql = "INSERT INTO statusled(tanggal, waktu, status, suhu) VALUES ( CURRENT_DATE(), CURRENT_TIME(), '%s', '%f')" % (statusled, temp)
				cursor.execute(sql)
				db.commit()
				db.close()
	except:
